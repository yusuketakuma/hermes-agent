...
hermes config set OPENROUTER_API_KEY --stdin < /path/to/private-key-file
```

:::tip
Use `--stdin` for credentials supplied by a pipe or private input file so the value stays out of process arguments. It cannot be combined with a positional value, removes one final newline, and rejects empty input.
