...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    Two gates hold a card past ``recompute_ready``: an explicit ``kanban_block``
    or ``result_unknown`` (both require an operator; ``unblock_task`` emits
    ``unblocked`` to clear either — #28712), or a breaker trip
    ``_record_task_failure`` stamped ``sticky`` — the clean-exit
    ...
    if row and row["kind"] in {"blocked", "result_unknown"}:
        return True
    trip = conn.execute(
    ...
    try:
        with write_txn(conn):
            ...
            trow = conn.execute(
                ...
                "worker_started_at, execution_scope FROM tasks WHERE id = ?", (task_id,),
            ).fetchone()
            if trow is None:
...
def invalidate_descendants_for_parent_reopen(
    ...
    "termination_records": [(task_id, worker_pid, claim_lock, worker_started_at)]}``.
    """
    caller_owns_txn = bool(conn.in_transaction)
    now = int(time.time())
    invalidated: list[dict[str, Any]] = []
    terminations: list[tuple[Optional[int], Optional[str], Optional[int]]] = []
    termination_records: list[tuple[str, Optional[int], Optional[str], Optional[int]]] = []
    with write_txn(conn, allow_nested=True):
        rows = conn.execute(
            """
            WITH RECURSIVE descendants(id) AS (
        ...
        for row in rows:
            ...
            if previous_status == "review":
                ...
                termination_records.append((row["id"], row["worker_pid"], row["claim_lock"], row["worker_started_at"]))
                run_id = _end_run(
    ...
    if not caller_owns_txn:
        ...
        for descendant_id, pid, claim_lock, started_at in termination_records:
            termination = _terminate_reclaimed_worker(pid, claim_lock, started_at=started_at)
            if _worker_survived_termination(termination):
