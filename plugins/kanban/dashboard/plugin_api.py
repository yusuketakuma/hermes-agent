...
def _set_status_direct(conn: sqlite3.Connection, task_id: str, new_status: str) -> bool:
    ...
    terminations: list[tuple[str, Optional[int], Optional[str], Optional[int]]] = []
    ...
    with kanban_db.write_txn(conn):
        ...
        if was_running and effective_status != "running" and prev["current_run_id"]:
            ...
            terminations.append((task_id, prev["worker_pid"], prev["claim_lock"], prev["worker_started_at"]))
        conn.execute(
    ...
    for terminated_task_id, pid, claim_lock, started_at in terminations:
        termination = kanban_db._terminate_reclaimed_worker(pid, claim_lock, started_at=started_at)
