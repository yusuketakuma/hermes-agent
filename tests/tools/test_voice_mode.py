...
class TestPulseSocketReachable:
    @staticmethod
    def _short_runtime_dir(tmp_path):
        """AF_UNIX caps sun_path at ~104 bytes; pytest's tmp_path under
        macOS /var/folders exceeds that, so bind sockets in a shallow dir."""
        import tempfile
        return Path(tempfile.mkdtemp(prefix="pulse-test-"))
    ...
    def test_stale_socket_file_not_reachable(self, monkeypatch, tmp_path):
        ...
        tmp_path = self._short_runtime_dir(tmp_path)
